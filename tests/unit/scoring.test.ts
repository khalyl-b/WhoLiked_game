...
describe("time-decay scoring", () => {
  ...
  it("starts at 1000 and decays linearly to 100", () => {
    expect(calculateCorrectGuessPoints({ startedAt: start, answerDeadline: deadline, submittedAt: start })).toBe(1000);
    expect(calculateCorrectGuessPoints({ startedAt: start, answerDeadline: deadline, submittedAt: "2026-08-15T12:00:15.000Z" })).toBe(550);
    expect(calculateCorrectGuessPoints({ startedAt: start, answerDeadline: deadline, submittedAt: "2026-08-15T12:00:29.999Z" })).toBe(100);
  });
  ...
  it("uses the same curve for the live points display", () => {
    ...
    expect(calculateDisplayedPotentialPoints(15_000, 30)).toBe(550);
    expect(calculateDisplayedPotentialPoints(1, 30)).toBe(100);
  });
});
